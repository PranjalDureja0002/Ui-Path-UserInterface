...
Backend URL + secret resolve in priority order:
  ...
  3. UiPath Assets  Foreman-Backend-Url / Foreman-Ingest-Secret  (cloud — set once in the tenant)
...
def _resolve() -> tuple[str, str]:
    ...
    if not backend:
        backend = _asset("Foreman-Backend-Url")
    if not secret:
        secret = _asset("Foreman-Ingest-Secret")
