...
def fleet_payload_v2(asset_id: str, failure_mode: str) -> dict[str, Any]:
    ...
    for r in rows:
        ...
        fkey = r["factor"]
        if fkey and fkey not in factors:
            ftype = _FACTOR_NODE_TYPE.get(r["factor_type"], "batch")
            is_hub = fkey in root_ids
            factors[fkey] = {"id": fkey, "label": fkey, "type": ftype, "hub": is_hub,
                             "status": "failing" if is_hub
                             else ("healthy" if ftype == "batch" else "at_risk")}
        if fkey:
            # an edge is "hot" only if it links to a culprit hub (a common-cause
            # root) — an install edge to a *healthy* crew is not a propagation path
            edges.append({"from": aid, "to": fkey, "rel": r["rel"],
                          "hot": bool(factors[fkey].get("hub"))})

    # status: failing if it exhibited (or is the origin); at_risk if it shares a
    # hot factor (crew/lot); healthy if it only shares a batch — that last group
    # is exactly the false positive a "same batch" query would wrongly flag.
    ...
    return {
        ...
        "sqlVsGraph": {
            "sqlFound": real,
            "sqlNote": f"WHERE batch = X -> {peers} same-batch {unit}s, {real} actually failed",
